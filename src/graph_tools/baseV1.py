...
def export_graph_to_txt(graph, output_path):
    with open(output_path, "w", encoding="utf-8") as output_file:
        ...
        for source, target, _, data in graph.edges(keys=True, data=True):
            highway_value = data.get("highway")
            metadata = json.dumps({"highway": highway_value}, ensure_ascii=True, default=str)
            output_file.write(f"{source} -> {target} | {metadata}\n")

# Download street network for a specific place
G = ox.graph_from_place("Bowling Green, Kentucky", network_type="drive")
